        ...
        <div className="mt-14 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {PRODUCTS.map((p, i) => (
            ...
            >
              ...
              <p className="font-body text-sm font-semibold text-[#FF4500] mt-1">
                {p.tagline}
              </p>
              <p className="font-body text-sm text-zinc-600 leading-relaxed mt-3 flex-1">
                {p.description}
              </p>
              <button
                onClick={() => go("contact")}
                data-testid={`product-enquire-${p.id}`}
                className="inline-flex items-center gap-1.5 font-body font-bold text-sm text-[#0A0A0A] mt-5 hover:text-[#FF4500] transition-colors"
              >
                Enquire
